...
app.post("/api/analyze-language", async (req, res) => {
  try {
   ...
   console.log("🧠 Gemini response status:", r);
    if (!r.ok) {
    ...
    const textOut = json?.candidates?.[0]?.content?.parts?.[0]?.text || "0";
    console.log("🧠 Gemini response text:", json);
    const m =
    ...
    if (!m) {
      return res.status(500).json({ error: "could_not_parse", raw: textOut });
    }
